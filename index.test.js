...
testdb( 'second test', async function() {
  ...
  expect( res.firstRow() ).toStrictEqual( {foo_id:1, bar_id:2, buz_id:1, foo_value:'1,2,1' } ) ;
...
testdb( 'third test', async function() {
  ...
  expect(res.firstRow()).toStrictEqual( {foo_id:0, bar_id:1, buz_id:7, foo_value:'0,1,7' } ) ;
...
afterAll(async() => {
  console.log( "shutdownPool() done" );
  (await DatabaseContext.shutdownPool());
});
